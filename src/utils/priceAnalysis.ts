...
import { daysBetweenIso, forwardFillDaily, isoWeekday, shiftIsoDay } from './dailySeries';
...
function daily(points: readonly PriceHistoryPoint[]): PriceHistoryPoint[] {
  const clean = points
    .filter((p) => typeof p.date === 'string' && Number.isFinite(p.price) && p.price > 0)
    .slice()
    .sort((a, b) => a.date.localeCompare(b.date));
  return forwardFillDaily(clean, (point, date) => ({ ...point, date }));
}
...
export function historyCoverageDays(points: PriceHistoryPoint[]): number {
  ...
  const span = daysBetweenIso(sorted[0].date, sorted[sorted.length - 1].date) + 1;
...
function priceNDaysAgo(points: readonly PriceHistoryPoint[], n: number): number | null {
  const expanded = daily(points);
  if (!expanded.length) return null;
  const target = shiftIsoDay(expanded[expanded.length - 1].date, -n);
  for (let i = expanded.length - 1; i >= 0; i -= 1) {
    if (expanded[i].date <= target) return expanded[i].price;
  }
  return expanded[0].price;
}

export function statsFor(points: PriceHistoryPoint[]): PriceStats | null {
  const expanded = daily(points);
  if (expanded.length < 2) return null;
  const prices = expanded.map((p) => p.price);
  const current = prices[prices.length - 1];
  const min = Math.min(...prices);
  const max = Math.max(...prices);
  const avg = prices.reduce((a, b) => a + b, 0) / prices.length;

  const window = expanded.slice(-CHEAP_LOOKBACK_DAYS);
  ...
  return {
    ...
    max,
    avg,
    change7dPct: pct(priceNDaysAgo(expanded, 7)),
    change30dPct: pct(priceNDaysAgo(expanded, 30)),
    low30,
  };
}
...
export function weekdayCycle(points: PriceHistoryPoint[]): WeekdayCycle | null {
  const expanded = daily(points);
  if (expanded.length < WEEKDAY_MIN_POINTS) return null;
  const sums = new Array<number>(7).fill(0);
  const counts = new Array<number>(7).fill(0);
  for (const p of expanded) {
    const day = isoWeekday(p.date);
    sums[day] += p.price;
    counts[day]++;
  }
...
export function forecast(points: PriceHistoryPoint[], horizon: number): ForecastResult | null {
  const expanded = daily(points);
  if (expanded.length < 2 || historyCoverageDays(expanded) < FORECAST_MIN_DAYS) return null;
  const window = expanded.slice(-TREND_WINDOW);
  ...
  const allMean = expanded.reduce((a, p) => a + p.price, 0) / expanded.length;
  const sums = new Array<number>(7).fill(0);
  const counts = new Array<number>(7).fill(0);
  for (const p of expanded) {
    const day = isoWeekday(p.date);
  ...
  const lastDay = isoWeekday(last.date);
  const predicted: PriceHistoryPoint[] = [];
  for (let d = 1; d <= horizon; d++) {
    const date = shiftIsoDay(last.date, d);
    const targetDay = isoWeekday(date);
    const value = clamp(last.price + slope * d + season[targetDay] - season[lastDay]);
    predicted.push({ date, price: value });
  }
...
export function isCheapDay(points: PriceHistoryPoint[]): CheapDay {
  const expanded = daily(points);
  if (expanded.length < 2) return { cheap: false, reason: null, dropPct: null };

  const current = expanded[expanded.length - 1].price;
  const window = expanded.slice(-CHEAP_LOOKBACK_DAYS);
  const low30 = Math.min(...window.map((p) => p.price));
  if (current <= low30) return { cheap: true, reason: 'low30', dropPct: null };
  const ref = priceNDaysAgo(expanded, 7);
  if (ref) {
